...
export default function KelolaMedisPage() {
  ...
  const fetchMedis = async () => {
    try {
      ...
      if (data.success) setMedisList(data.data);
    } catch (err) {
    ...
    <div style={{ padding: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>Buat dan kelola akses untuk tenaga medis.</p>
        </div>
          ...
          <tbody>
            {medisList.map((medis) => (
              <tr key={medis.id_user}>
                ...
                <td><span className={styles.badge} style={{ backgroundColor: '#e2e8f0', color: '#475569' }}>Tenaga Medis</span></td>
